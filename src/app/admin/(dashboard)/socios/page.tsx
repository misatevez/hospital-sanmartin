...
export default async function SociosPage() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-[#1a3a5c] mb-8">Socios</h1>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-[#f8fafc]">
                <th className="text-left px-6 py-4 text-[#6b7280] font-medium">Socio</th>
            ...
            <tbody>
              ...
              {(socios ?? []).map((s) => {
                const badge = STATUS_LABEL[s.status] ?? { label: s.status, className: 'bg-gray-100 text-gray-500' }
                return (
                  <tr key={s.id} className="border-b border-gray-50 hover:bg-[#f8fafc] transition-colors">
                    <td className="px-6 py-4">
                      <p className="font-medium text-[#1a3a5c]">{s.nombre} {s.apellido}</p>
                      <p className="text-[#9ca3af] text-xs">{s.email}</p>
                    </td>
                    <td className="px-6 py-4 text-[#1f2937]">{s.plan}</td>
