...
export default async function SocioDetallePage({ params }: { params: { id: string } }) {
  ...
  const [{ data: socio }, { data: pagos }, { data: comprobantes }] = await Promise.all([
    supabase.from('subscriptions').select('*').eq('id', params.id).single(),
    supabase.from('payments').select('*').eq('subscription_id', params.id).order('created_at', { ascending: false }),
    supabase.from('receipts').select('*').eq('subscription_id', params.id).order('created_at', { ascending: false }),
  ])

  if (!socio) notFound()

  // Cross-reference: check if this person also made a one-time donation
  const { data: donaciones } = await supabase
    .from('donations')
    .select('id, monto, status, created_at')
    .eq('email', socio.email)
    .order('created_at', { ascending: false })

  const badge = STATUS_LABEL[socio.status] ?? { label: socio.status, className: 'bg-gray-100 text-gray-500' }

  return (
    <div className="max-w-3xl">
      {/* Breadcrumb */}
      <div className="flex items-center gap-2 text-sm text-[#9ca3af] mb-6">
        <Link href="/admin/socios" className="hover:text-[#2563a8]">Socios</Link>
        <span>/</span>
        <span className="text-[#1a3a5c] font-medium">{socio.nombre} {socio.apellido}</span>
      </div>
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 mb-6">
        <div className="flex items-start justify-between mb-4">
          <div>
            ...
            <p className="text-[#6b7280] text-sm">{socio.email}</p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t border-gray-100">
          <div>
            ...
            <p className="text-sm font-medium text-[#1f2937]">{socio.telefono}</p>
          </div>
          <div>
            <p className="text-xs text-[#9ca3af] mb-1">Plan</p>
            <p className="text-sm font-medium text-[#1f2937]">{socio.plan}</p>
          </div>
          <div>
            <p className="text-xs text-[#9ca3af] mb-1">Monto mensual</p>
            <p className="text-sm font-medium text-[#2563a8]">
              ${Number(socio.monto).toLocaleString('es-AR')}
            </p>
          </div>
          <div>
      ...
      {/* Donaciones únicas (cross-reference) */}
      {(donaciones ?? []).length > 0 && (
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 mb-6">
          <h2 className="text-base font-semibold text-[#1a3a5c] mb-4">Donaciones únicas realizadas</h2>
              ...
              <div key={d.id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${
                    ...
                    {d.status === 'approved' ? 'Acreditada' : d.status === 'rejected' ? 'Rechazada' : 'Pendiente'}
                  </span>
                </div>
              </div>
            ))}
